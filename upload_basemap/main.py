...
def main():
    ...
    try:
        ...
        new_files = sum(1 for file_path, prefix in files_and_prefixes 
                       if not tracker.is_uploaded(file_path, prefix))
        
        logging.info(f"Found {total_files} total files, {new_files} new files to upload")
        ...
        for file_path, prefix in files_and_prefixes:
            try:
                ...
                if tracker.is_uploaded(file_path, prefix):
                    ...
                    logging.info(f"Skipping {file_path} - already uploaded on {upload_info['uploaded_at']}")
                    continue
